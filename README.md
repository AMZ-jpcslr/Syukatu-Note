...
2. SQL Editorで **`supabase/migrations/202609220003_v11.sql` だけ** を全文実行します。001・002を適用済みなら再実行しません。003はトランザクション内の列／テーブル追加・制約拡張・関数置換で、既存テーブル・行を削除しません。列追加後に旧CSV RPCを更新するため、必ず全文を適用してください。
