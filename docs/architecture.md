...
拡張は `activeTab` でクリックされたページだけを読み、端末内でサニタイズ・ルール解析します。取り込み専用トークンで送るのは抽出結果だけです。Webの既存SupabaseセッションやCookieを拡張へ渡しません。

| テーブル                                   | 用途                                                   |
| ------------------------------------------ | ------------------------------------------------------ |
| `data_sources` / `import_inbox`            | 本人だけの取得元・候補・前回の抽出結果                 |
| `imported_events`                          | 承認済みの日付/日時を区別するカレンダー予定            |
| `import_audit`                             | 承認した項目・連携先・上書き選択の履歴                 |
| `extension_pairings` / `extension_tokens`  | 一回限りのコードと端末トークンのハッシュ。サーバー専用 |
| `gmail_connections` / `gmail_oauth_states` | 暗号化したOAuth認証情報と一回限りのstate。サーバー専用 |
| `user_profiles`                            | 本人のスキル・経験・関心タグ                           |

`review_private_import` は所有者を検証し、選ばれた日程・タスク・フローを一括反映します。取り込み処理から公開テンプレートを変更する経路はありません。各重要フィールドの `field_provenance` は出典・証拠・更新日時・手動修正を保持します。既存の企業/選考から派生するカレンダーに、承認済み `imported_events` を加えています。

拡張・Gmail・引き継ぎ時の認証失効・導入上の制約は [v2手順](v2.md) を参照してください。
